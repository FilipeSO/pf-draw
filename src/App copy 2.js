...
export const App = () => {
  ...
  useEffect(() => {
    ...
    for (let key in _BARS) {
      ...
      // preparar dados equipamentos da barra
      ...
      nodeEquips.forEach((nodeEquip) => {
        ...
        if (nodeEquip.type === "LT") {
          ...
          let n = orgEquips.filter((equip) => equip.name === equipName).length;
          let dx = Math.cos(((90 + 25 * (n - 1)) * Math.PI) / 180) * 40;
          let dy = Math.sin(((90 + 25 * (n - 1)) * Math.PI) / 180) * 40;

          orgEquips.push({
            ...nodeEquip,
            pos: {
              x: rndBars[nodeEquip.endPointA].pos.x + dx,
              y: rndBars[nodeEquip.endPointA].pos.y + dy,
            },
            n: 1 + n,
          });
        }
